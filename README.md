...
- 5분 스피드·10분 집중 모드와 끝까지 생존하는 승리 조건
- 15분 분량의 20개 웨이브를 모드 길이에 맞춰 압축한 고밀도 스폰
- 레벨업 창의 실시간 템포는 유지하고 선택 한 번당 3단계 성장
...
`pnpm test:e2e`에는 가속된 5분 전체 타임라인, 모바일 터치 조이스틱, PWA 오프라인 재실행 검증이 포함됩니다. 전체 브라우저 검증은 약 1분 정도 걸립니다.
...
- `5분 스피드`: 기존 15분 웨이브를 3배속으로 압축하고 생성 밀도를 3배로 높입니다.
- `10분 집중`: 같은 웨이브를 1.5배속으로 압축하고 생성 밀도를 1.5배로 높입니다.
- 두 모드 모두 제한 시간이 끝날 때 살아 있으면 승리합니다.
- 경험치 획득량을 압축률에 맞춰 보정해 레벨업 선택창의 실시간 빈도는 기존과 비슷하게 유지합니다.
- 업그레이드 선택 한 번은 무기 또는 패시브를 최대 3단계 올립니다.
