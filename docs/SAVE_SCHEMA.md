...
`RunState`에는 캐릭터, 5분/10분 모드 ID, 경과 시간, 시드, 점수, 처치, 레벨/경험치, 필살기, 무기, 보조 능력, 계산된 플레이어 능력치와 처치한 보스가 들어갑니다. 계산된 능력치는 호환용 캐시이며, 복원 시 캐릭터와 보조 능력 레벨을 기준으로 현재 공식에서 다시 계산합니다.
...
- 모드 필드가 없는 이전 v2 저장은 5분 스피드 모드로 안전하게 기본 지정합니다.
